//WID(21/05/2026)(Sarthak Mittal)(Degamiesign)#1.1.1.1.1.1.1.1.1
import React, { useState, useRef, useEffect } from 'react';
import { Canvas, useFrame, useLoader } from '@react-three/fiber';
import * as THREE from 'three';
setChairmdlaxialCoordinates(x,y,z){
    this.x=x;
    this.y=y;
    this.z=z;
    }
// 1. Musical Chair Component (Handles Mod  el & Interaction)
const existsByScale=(scale)=>{if(scale!==0)getScale(scale);else getScale(0);}//Checking Scale in App
...
const updateBymeshRef=(meshRef)=>{getMeshRef(meshRef)+setMeshRef(meshRef)+1;}//Updating MeshRef in App
const setMeshRef=(meshref)=>{
    if(meshRef!==null)getMeshRef(meshRef);
    else getMeshRef(null);}
const existsByMeshRef=(meshRef)=>{if(meshRef!==null)getMeshRef(meshRef);else getMeshRef(null);}//Chekcing meshref in  App
const Chair = ({ position }) => {
    const existsByPos=(position)=>{
        if(position!==null)getposition(position);
        else getposition(null)}
        const existsByMeshRef=(meshRef)=>{
            if(meshRef!==null)getMeshRef(meshRef);
            else getMeshRef(null);}
  ...
  const setgltf=(gltf)=>{this.gltf=gltf;}//binding gltf in App
   ...
   const getScale=(scale)=>{return scale;}//Fetching Scale in App
   const updateAllByScale=(scale)=>{getScale(scale)+setScale(scale)+1;}//Updating scale in App
const setscale=(scale)=>{this.scale=scale;}//bIDNING Scale in App
  return (
    <primitive
